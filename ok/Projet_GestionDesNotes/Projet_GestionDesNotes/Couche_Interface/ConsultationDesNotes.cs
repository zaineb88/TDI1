...
using Projet_Maha_Et_Aouatif_TARIAF.Couche_Interface;
namespace Projet_Maha_Et_Aouatif_TARIAF.Couche_Interface
{
    public partial class ConsultationDesNotes : Form
    {
        ...
        private void comboBox1_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            {
                if (comboBox1.SelectedItem.ToString() == Program.eleve.L1[i].Matricule.ToString())
                {

                    label6.Text = Program.eleve.L1[i].Matricule.ToString();
                    label7.Text = Program.eleve.L1[i].Nom;
                    label8.Text = Program.eleve.L1[i].Prénom;
                    label10.Text = Program.eleve.L1[i].Adresse;
                    label9.Text = Program.eleve.L1[i].DateDeNaissance.ToShortDateString();
        ...
        private void EtatInitial()
        {
            if (Program.eleve.NombreEléveParticipant > 0)
            {
                ...
                for (int i = 0; i < Program.eleve.L1.Count; i++)
                {
                    comboBox1.Items.Add(Program.eleve.L1[i].Matricule);
            ...
            for (int i = 0; i < Program.eleve.NombreEléveParticipant; i++)
            {
                for (int j = 0; j < GestionDesNoteDuEléves.Mcs.NombreNote; j++)
                {
                    ...
                    for (int k = 0; k < 8; k++)
                    {
                        ...
                        if (GestionDesNoteDuEléves.Mcs.RechercheDoublon(Program.eleve.L1[i].Matricule, mat))
                        {

                            note += GestionDesNoteDuEléves.Mcs.Recherchenote(Program.eleve.L1[i].Matricule, mat).Note;
